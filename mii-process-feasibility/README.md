...
# General settings for the feasibility processes independent from the network or store settings 
general:
  # Maximum time duration between creation (status 'requested') of a feasibilityRequest task and its execution (status
  # 'in-progress') by the BPE.
  # Older unprocessed feasibilityRequest task will be rejected.
  # The duration is required to be given in the ISO 8601 format, e.g. "PT1H30M10S" (see
  # https://en.wikipedia.org/wiki/ISO_8601#Durations).
  # Defaults to 5 minutes.
  requestTaskTimeout: PT5M
```
