...
Besides the [common DSF settings controlled by different environment variables][8], there is one additional specific to this process.
...
The configuration file is a YAML formatted text file containing the configuration for the FHIR store(s) which will be
used for executing feasibility queries. It is separated into 2 main parts:
...
> [!IMPORTANT]
> The configuration file needs to be accessible by the BPE process where the feasibility process plugin is deployed. If
> you are using docker it needs to be mounted inside the BPE docker container. The path to this file has to be
> configured for the feasibility process plugin using the environment variable shown in the section
...
relation to the now unsupported environment variables used in versions before 1.0.1.0:
...
stores:
  ...
  my-store-1:
    ...
    # NOTE: trusted certificates are now needed to be provided as a single PEM file containing the concatenated
    #       certificates. Before it had to be provided in the PKCS#12 format (.p12).
    ...
    #       Before it had to be provided in the PKCS#12 format (.p12)
...
> [!IMPORTANT]
> Each store configuration in the section `stores` can only use **one** of the following authentication methods: `basicAuth`,
> `bearerAuth` or `oAuth`. A client certificate can be combined with each of the other authentication methods.

The following code block shows an example configuration for 2 parent organizations and 2 FHIR stores using TLS encrypted
communication with a self-signed server certificate, one requiring OpenID Connect authentication and CCDL queries and
the other a client certificate and CQL queries. One of the stores and the OpenID Connect provider are only accessible
via a forward proxy using basic authentication:
...
The format for enviromnent variables inside the YAML configuration to be recognized is by wrapping the variable name
with `${}`, e. g. by setting a value for the environment variable `BASE_URL` all occurences of the text `${BASE_URL}` in
the configuration YAML will be substituted with the value. The following configuration snippet shows the usage:
...
> [!NOTE]
> It is suggested to add an unique prefix to your custom environment variable names to prevent conflicts with existing
