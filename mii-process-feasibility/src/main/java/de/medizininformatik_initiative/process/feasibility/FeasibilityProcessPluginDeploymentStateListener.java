            ...
            if (storeClients.size() > 0 || flareWebserviceClients.size() > 0) {
                ...
                flareWebserviceClients.entrySet().parallelStream().forEach(f -> {
                    try {
                        ...
                        logger.info("Feasibility plugin connection test to Flare of FHIR store '{}' SUCCEEDED.",
                                f.getKey());
                    } catch (Exception e) {
                        logger.error("Feasibility plugin connection test to Flare of FHIR store '{}' FAILED."
                                + " Error: {} - {}", f.getKey(), e.getClass().getName(), e.getMessage());
                    }
                });
            } else {
