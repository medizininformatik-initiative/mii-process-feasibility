...
public class FlareWebserviceClientImplRevProxyTlsClientCertIT extends FlareWebserviceClientImplBaseIT {
    ...
    private static URL feasibilityConfig = getResource("reverseProxy_tls_clientCert.yml");
    ...
    static void dynamicProperties(DynamicPropertyRegistry registry) throws IOException {
        ...
        var config = createTempConfigFile(readFile(feasibilityConfig)
                .replaceAll("<<baseUrl>>", "https://%s:%d".formatted(proxyHost, proxyPort))
                .replaceAll("<<trustedCAFile>>", trustStoreFile.getPath())
                .replaceAll("<<clientCertificateFile>>", clientCertificateFile.getPath())
                .replaceAll("<<privateKeyFile>>", privateKeyFile.getPath())
